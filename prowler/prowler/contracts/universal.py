...
COMPLIANCE_SCOPE_OPTIONS: tuple[ScopeOption, ...] = tuple(
    ...
    for framework, provider in (parse_compliance_literal(literal),)
)
...
class UniversalProwlerContract(SelectionScopedContract):
    ...
    def build_provider_fields(self) -> list[ContractElement]:
        ...
        for service_provider, service_key in SERVICE_KEYS.items():
            fields.append(
                self._build_service_scope_select(service_provider, service_key)
            )
        fields.append(self._build_compliance_scope_select())
        return fields

    @staticmethod
    def _build_provider_select() -> ContractSelect:
        """Build the first, mandatory provider selection field."""
    ...
    @staticmethod
    def _build_service_scope_select(
        service_provider: str, service_key: str
    ) -> ContractSelect:
        """Build one provider-conditioned optional service select."""
        return ContractSelect(
            key=service_key,
    ...
    @staticmethod
    def _build_compliance_scope_select() -> ContractSelect:
        """Build the final optional compliance select."""
        return ContractSelect(
            key=COMPLIANCE_KEY,
    ...
    def parse_input(self, raw_input: Mapping[str, object]) -> ProviderInput:
        ...
        self._validate_scope_selection(
            selected_provider, service_key, service_route, compliance_route
        )
    ...
    def _validate_scope_selection(
        ...
        compliance_route: str | None,
    ) -> None:
        """Reject mutually exclusive or provider-mismatched scope selections."""
        if service_route is not None and compliance_route is not None:
            ...
            raise selection_input_error((service_key, COMPLIANCE_KEY), "scope_conflict")
        for key, route, options in (
            ...
            if (
                ...
                raise selection_input_error((key,), "scope_provider_mismatch")

    @staticmethod
    def _build_provider_candidate(
        raw_input: Mapping[str, object], selected_provider: str
    ) -> dict[str, object]:
        """Remove selection fields and normalize values for strict provider parsing."""
        foreign_keys = {
            key
    ...
    def _parse_provider_select(raw_input: Mapping[str, object]) -> str:
        ...
        selected = parse_closed_select(
            raw_input, PROVIDER_KEY, PROVIDER_SELECT_VALUES, required=True
        )
        if selected is None:
    ...
    def _parse_scope_select(
        ...
        if key is None:
            return None
        return parse_closed_select(
            raw_input, key, options, required=False, none_value=NONE_SCOPE
        )
