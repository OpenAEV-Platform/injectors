"""Local deterministic CHK.011 fixtures."""
...
def aws_form() -> dict[str, object]:
    """Return valid placeholder-only AWS form input."""
    return {
...
def ocsf_record_factory() -> Any:
    """Build one complete minimal OCSF record."""

    def build(
        title: str, *, provider: str = "aws", status: str = "FAIL"
    ) -> dict[str, Any]:
        if status == "PASS":
