...
```python
from injectors_second_sdk import BaseInjector
...
```python
from injectors_second_sdk import InjectorConfig
...
```python
from injectors_second_sdk import InjectorContext
...
```python
from injectors_second_sdk import ExecutionStatus
...
```python
from injectors_second_sdk import ExecutionCallback, ExecutionStatus
...
| Concern | Owner | Why |
|---|---|---|
| Lifecycle shape (`process_message`, `start`, `stop`) | `injectors-second-sdk` | Injector-specific, shared by all injectors |
| `DaemonProtocol` (`start`, `set_callback`, `get_id`) | `xtm-oaev-sdk` (re-exported) | Shared behavioral contract for daemon runtimes |
| Config field names and validation | `injectors-second-sdk` | Consistent across all injectors |
| Message parsing (`InjectorContext`) | `injectors-second-sdk` | Same nested dict traversal everywhere |
| Execution callback structure | `injectors-second-sdk` | Same payload shape everywhere |
