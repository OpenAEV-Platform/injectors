...
Detailed feature documentation for the injectors-second-sdk CLI Engine: architecture, public interface, dependency injection model, adapter reference, advanced nmap example, and error semantics.
...
- Every symbol in `__all__` is importable from `injectors_second_sdk`
...
```
Your code
  └─ imports from: injectors_second_sdk       (top-level __all__ — 30 symbols)
...
```python
from injectors_second_sdk import CliEngine, ExecPolicy, create_cli_engine
