...
```bash
uv add injectors-second-sdk
```

Or with pip:

```bash
pip install injectors-second-sdk
```
...
```python
from injectors_second_sdk import CliEngine, CommandSpec, CliContractError
```

**DDD-aware consumers** may import from the `contracts/` layer to express feature-scoped dependencies explicitly:

```python
from injectors_second_sdk.contracts.base_injector import BaseInjector, InjectorConfig
from injectors_second_sdk.contracts.cli_engine.ports import CommandExecutorPort
from injectors_second_sdk.contracts.cli_engine.contracts import CommandSpec, ExecPolicy
from injectors_second_sdk.contracts.common import CliError, CliExecutionError
```
...
from injectors_second_sdk._core.cli_engine.ports import CommandExecutorPort
```
...
from injectors_second_sdk._core.cli_engine.core.cli_engine import CliEngine
```
...
from injectors_second_sdk import BaseInjector, InjectorConfig
```
...
# Type checking
uv run mypy --strict injectors_second_sdk

# Linting
uv run ruff check injectors_second_sdk
```
