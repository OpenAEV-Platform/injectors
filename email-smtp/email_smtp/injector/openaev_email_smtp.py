...
class EmailSMTPInjector:
    def __init__(self, config: ConfigLoader, helper: OpenAEVInjectorHelper):
        ...
        self.helper.injector_logger.info(
            f"{LOG_PREFIX} - Email SMTP injector initialized"
        )
    ...
    def execute(
        ...
        if inject_contract != EmailContractId.CRAFT_EMAIL:
            raise InvalidContractError("Unsupported contract for Email SMTP injector")
    ...
    def start(self) -> None:
        self.helper.injector_logger.info(
            f"{LOG_PREFIX} - Starting email SMTP injector listener"
        )
