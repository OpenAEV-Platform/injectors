...
- [OpenAEV Email (SMTP) Injector](#openaev-email-smtp-injector)
  ...
  - [Behavior](#behavior)
  - [Debugging](#debugging)
  - [Migration from Email](#migration-from-email)

## Introduction

OpenAEV (Breach and Attack Simulation) drives injectors to execute the technical actions of a scenario. The Email
(SMTP) injector registers an email contract with the OpenAEV platform; when an inject using this contract is played, OpenAEV
...
```mermaid
flowchart LR
    O[OpenAEV inject] -->|job via RabbitMQ| I(Email SMTP injector)
