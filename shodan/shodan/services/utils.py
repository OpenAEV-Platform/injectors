...
class Utils:
    ...
    def _make_json(self, data, output_trace_config: dict):
        options_show_json_is_active = self._get_trace_config(output_trace_config, "options.show_json.is_active", False)
        if not options_show_json_is_active:
            return None

        options_show_json_indent = self._get_trace_config(output_trace_config, "options.show_json.indent", 2)
        options_show_json_sort_keys = self._get_trace_config(output_trace_config, "options.show_json.sort_keys", False)
    ...
    def _make_separator(self, output_trace_config: dict) -> Rule | None:
        options_show_separator = self._get_trace_config(output_trace_config, "options.show_separator.is_active", default=False)
    ...
    # MAKE SECTION CONFIG
    def _make_section_config(self, output_trace_config: dict, data_sections_config: list[dict]):
        options_show_sections = self._get_trace_config(output_trace_config, "options.show_sections.is_active", default=True)
        options_show_sec_config = self._get_trace_config(output_trace_config, "options.show_sections.sec_config", default=True)

        if not (options_show_sections and options_show_sec_config):
            return None

        sec_config_icon = self._get_output_icon(
            self._get_trace_config(output_trace_config, "sections_config.header.icon", default="CONFIG")
        )
        sec_config_title = self._get_trace_config(output_trace_config, "sections_config.header.title", default="[CONFIG] Summary of all configurations used for the contract.")
        keys_list_to_string = self._get_trace_config(output_trace_config, "sections_config.keys_list_to_string", default=[])
        keys_to_exclude = self._get_trace_config(output_trace_config, "sections_config.keys_to_exclude", default=[])

        section_tree = Tree(f"{sec_config_icon} {sec_config_title}".strip(), guide_style="bold")
        self._build_tree(
            tree=section_tree,
    ...
    # MAKE SECTION INFO
    def _make_section_info(self, output_trace_config: dict, data_sections_info: list[dict]):
        options_show_sections = self._get_trace_config(output_trace_config, "options.show_sections.is_active", default=True)
        options_show_sec_info = self._get_trace_config(output_trace_config, "options.show_sections.sec_info", default=True)

        if not (options_show_sections and options_show_sec_info):
            return None

        sec_info_icon = self._get_output_icon(
            self._get_trace_config(output_trace_config, "sections_info.header.icon", default="INFO")
        )
        sec_info_title = self._get_trace_config(output_trace_config, "sections_info.header.title", default="[INFO] The Injector information")
        keys_list_to_string = self._get_trace_config(output_trace_config, "sections_info.keys_list_to_string", default=[])
        keys_to_exclude = self._get_trace_config(output_trace_config, "sections_info.keys_to_exclude", default=[])

        section_tree = Tree(f"{sec_info_icon} {sec_info_title}".strip(), guide_style="bold")
        self._build_tree(
            tree=section_tree,
    ...
    def _prepare_call_details(self, output_trace_config:dict, data_sections_external_api: list[dict]):
        results_success = []
        ...
        for result in data_sections_external_api:
            ...
            if result.get("is_error"):
                ...
                count_at_path = self._get_trace_config(output_trace_config, "sections_external_api.call_success.count_at_path", default="")
                if count_at_path:
                    total_success_details_count += self._count_at_path(call_success_details.get("result"), count_at_path)

        return (results_success, total_success_details_count), (results_failed, total_call_failed_count)

    # MAKE SECTION EXTERNAL API
    def _make_section_external_api(self, output_trace_config: dict, data_sections_external_api: list[dict]):
        options_show_sections = self._get_trace_config(output_trace_config, "options.show_sections.is_active", default=True)
        options_show_sec_external_api = self._get_trace_config(output_trace_config, "options.show_sections.sec_external_api", default=True)

        if not (options_show_sections and options_show_sec_external_api):
            return None

        sec_external_api_icon = self._get_output_icon(
            self._get_trace_config(output_trace_config, "sections_external_api.header.icon", default="API")
        )
        sec_external_api_title = self._get_trace_config(output_trace_config, "sections_external_api.header.title", default="[INJECTOR] Call API completed")
        section_tree = Tree(f"{sec_external_api_icon} {sec_external_api_title}".strip(), guide_style="bold")

        # Add node call success
        call_success_icon = self._get_output_icon(
            self._get_trace_config(output_trace_config, "sections_external_api.call_success.icon", default="SUCCESS")
        )
        call_success_title = self._get_trace_config(output_trace_config, "sections_external_api.call_success.title", default="Call Success")
        success_node = section_tree.add(f"{call_success_icon} {call_success_title}".strip())

        # Add node call failed
        call_failed_icon = self._get_output_icon(
            self._get_trace_config(output_trace_config, "sections_external_api.call_failed.icon", default="FAILED")
        )
        call_failed_title = self._get_trace_config(output_trace_config, "sections_external_api.call_failed.title", default="Call Failed")
        failed_node = section_tree.add(f"{call_failed_icon} {call_failed_title}".strip())

        (results_success_details, total_success_details_count), (results_failed_details, total_call_failed_count) = self._prepare_call_details(output_trace_config, data_sections_external_api)
        ...
        for result_detail in results_success_details:
            ...
            count_at_path = self._get_trace_config(output_trace_config, "sections_external_api.call_success.count_at_path", default="")
            if count_at_path:
                result_details_count += self._count_at_path(result_detail.get("result"), count_at_path)
            target_node = success_details_node.add(f"• {target} → {result_details_count} results")
    ...
    def _extract_level(self, items: list[Any], parts_remaining: list[str], use_key: bool = False) -> list[Any]:
        ...
        for item in items:
            if isinstance(item, dict):
                if part == "*":
                    ...
                    if next_parts:
                        sub_result = self._extract_level(sub_result, next_parts, use_key)
                ...
                if part == "*" and next_parts:
                    result.extend(self._extract_level([items[item]], next_parts, use_key))
                else:
    ...
    def _extractor(self, data: list[dict], path: str, use_key: bool = False) -> list[list[Any]]:
        if not path:
            return []
    ...
    @staticmethod
    def _organizer_row(tables_config_columns, show_index_is_active, index_start, column_values, column_extras):
        ...
        for row_idx in range(row_count):
            ...
            for table_config_index, table_config in enumerate(tables_config_columns):
                ...
                values = column_values[table_config_index] if table_config_index < len(column_values) else []
                extras = column_extras[table_config_index] if column_extras and table_config_index < len(column_extras) else None
                ...
                if mode == "single":
                    ...
                    if row_idx == 0:
                        ...
                        for val in values:
                            if isinstance(val, list):
                                all_vals.extend(str(v) for v in val if v not in (None, ""))
                    ...
                    if row_idx < len(values):
                        ...
                        if isinstance(val, list):
                            if val:
                                if extra:
                                    cell = ", ".join(f"{v} ({e if e is not None else '-'})" for v, e in zip(val, extra or ["-"]*len(val)))
                                else:
    ...
    def _rows_with_limit_cells(rows:list[list[str]], max_display_by_cell:int | None):
        ...
        for row in rows:
            ...
            for cell in row:
                if isinstance(cell, str) and "," in cell and max_display_by_cell is not None:
    ...
    # MAKE TABLES
    def _make_tables(self, output_trace_config: dict, data_tables: list[dict], auto_create_assets:bool | None):

        options_show_tables_is_active = self._get_trace_config(output_trace_config, "options.show_tables.is_active", default=True)
        if not options_show_tables_is_active:
            return []

        show_index_is_active = self._get_trace_config(output_trace_config, "options.show_tables.show_index.is_active", default=False)
        index_start = self._get_trace_config(output_trace_config, "options.show_tables.show_index.index_start", default=0)
        show_lines = self._get_trace_config(output_trace_config, "options.show_tables.show_lines", default=True)
        max_display_by_cell = self._get_trace_config(output_trace_config, "options.show_tables.max_display_by_cell", default=10)
        ...
        for table_config in tables:
            ...
            header_title = self._get_trace_config(table_config, "header.title", default="")
            search_entity = self._get_trace_config(table_config, "config.search_entity", default=None)
            tables_final = []
            for data_table in data_tables:
                ...
                tables_config_columns = self._get_trace_config(table_config, "config.columns")
                ...
                rows_with_limit_cell = self._rows_with_limit_cells(final_rows, max_display_by_cell)
    ...
    def generate_output_message(
        ...
        output_sections_client_api, results_success_details = self._make_section_external_api(
            output_trace_config=output_trace_config,
