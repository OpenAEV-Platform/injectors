...
| Parameter      | config.yml                  | Docker environment variable | Default | Mandatory | Description                                                                    |
|----------------|-----------------------------|-----------------------------|---------|-----------|--------------------------------------------------------------------------------|
| Hash Algorithm | `email.hash_algorithm` | `EMAIL_HASH_ALGORITHM` | sha256  | No        | Hash algorithm used for signature generation (URL and attachment hashes). One of `sha256`, `sha1`, `md5`. |
...
```shell
poetry install
poetry run python -m openaev_email
```
