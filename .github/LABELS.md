...
- `scope` is optional — a **lowercase** noun in parentheses naming the affected
  area. In this repository, use the **lowercase injector name** as the scope
  (e.g. `feat(http-query):`, `fix(caldera):`). The scope **replaces** the old
  `[injector]` bracket prefix, which is **discontinued**.
